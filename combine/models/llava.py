...
from combine.models.qwen2 import CombineQwen2ForCausalLM,CombineQwen2Model

class LlavaQwenConfig(Qwen2Config):
    model_type = "llava_qwen"


class CombineLlavaQwenModel(LlavaMetaModel, CombineQwen2Model):
    config_class = LlavaQwenConfig

    def __init__(self, config: Qwen2Config):
        super(CombineLlavaQwenModel, self).__init__(config)


class CombineLlavaQwenForCausalLM(CombineQwen2ForCausalLM, LlavaMetaForCausalLM):
    config_class = LlavaQwenConfig
    ...
    def __init__(self, config):
        ...
        config.update(dict_)
        # print(config)
        CombineQwen2ForCausalLM.__init__(self, config)
        config.model_type = "llava_qwen"
        config.rope_scaling = None

        self.model = CombineLlavaQwenModel(config)
    ...
    def prepare_layerwise_feature_extraction(        
        ...
        if inputs_embeds is None:
            (
                ...
                images, 
                modalities, 
                [(384,384)] * input_ids.shape[0]
            )
        return super().prepare_layerwise_feature_extraction(
            input_ids=input_ids,
    ...
    def get_layerwise_features(self,all_hidden_states, all_self_attns, output_hidden_states, 
                               hidden_states, attention_mask, 
                               position_ids,past_key_values,output_attentions,
                               use_cache,cache_position_vlm,layer_idx):
        return super().get_layerwise_features(
    ...
    def return_token_scores(self,all_hidden_states, all_self_attns, next_decoder_cache, hidden_states,labels,use_cache,return_dict,use_legacy_cache,rope_deltas):
        return super().return_token_scores(all_hidden_states, all_self_attns, next_decoder_cache, hidden_states,labels,use_cache,return_dict,use_legacy_cache)
    ...
    def _get_stopping_criteria(
        ...
        if("_eos_token_tensor" in vars(generation_config)):
            if generation_config._eos_token_tensor is not None:
                criteria.append(EosTokenCriteria(eos_token_id=generation_config._eos_token_tensor))
        if("eos_token_id" in vars(generation_config)):
    ...
    def _prepare_generation_config(
        ...
        # print(generation_config,kwargs)
    ...
    def prepare_generation_params(
        ...
        **kwargs,
        ...
        # 
        return [input_ids, generation_config, model_kwargs, generation_mode, prepared_stopping_criteria,prepared_logits_processor]
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, inputs_embeds=None, **kwargs):
        ...
        inputs = super().prepare_inputs_for_generation_old(input_ids, past_key_values=past_key_values, inputs_embeds=inputs_embeds, **kwargs)
    ...
    def run(
        ...
        **kwargs,
        ...
        # print(super())
        return self.generate(position_ids=position_ids, attention_mask=attention_mask, inputs_embeds=inputs_embeds, **kwargs)
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        for k, v in overwrite_config.items():
                            setattr(llava_cfg, k, v)
                        model = CombineLlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
                    else:
                        model = CombineLlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, **kwargs)
