...
def run_segmentation_tracking(video_predictor, image_predictor, processor, grounding_model, text, video_dir, output_dir,output_video_path):
    # create the output directory
    device = "cuda" if torch.cuda.is_available() else "cpu"
    CommonUtils.creat_dirs(output_dir)
    mask_data_dir = output_dir#os.path.join(output_dir, "mask_data")
    ...
    segments = np.full([5, image.size[-1], image.size[-2]], 10000,dtype=np.int16)
    ...
    # prompt grounding dino to get the box coordinates on specific frame
        ...
        for frame_idx, frame_masks_info in video_segments.items():
            ...
            segments[frame_idx] = mask_img


            # np.save(os.path.join(mask_data_dir, frame_masks_info.mask_name), mask_img)

            # json_data = frame_masks_info.to_dict()
            # # print(json_data_dir, frame_masks_info.mask_name.replace(".npy", ".json"))
            # json_data_path = os.path.join(json_data_dir, frame_masks_info.mask_name.replace(".npy", ".json"))
            # with open(json_data_path, "w") as f:
            #     json.dump(json_data, f)
    segments = segments[:, ::2, ::2]
    np.save(os.path.join(mask_data_dir, "segment_array.npy"), segments.astype(np.int16))
